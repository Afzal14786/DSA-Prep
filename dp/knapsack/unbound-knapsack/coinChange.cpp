...
public:
    int minCoins(vector<int>& coins, int amount) {
        ...
        int minNoOfCoins = 1e9;

        for (int i = 0; i < coins.size(); ++i) {
            int res = minCoins(coins, amount - coins[i]);
            if (res != 1e9) 
                minNoOfCoins = min(minNoOfCoins, res + 1);
        }
...
public:
    int coinChange(vector<int>& coins, int amount) {
        ...
        for (int i = 1; i <= amount; ++i) {
            for (int j = 0; j < n; ++i) {
                dp[i] = min(dp[i], 1 + dp[i - coins[i-1]]);
            }
        }
...
public:
    int coinChange(vector<int>& coins, int amount) {
        ...
        // If we have 0 coins, we can't make any amount > 0
        for (int j = 1; j <= amount; ++j) dp[0][j] = INF;

        for (int i = 1; i <= n; ++i) {
            for (int j = 1; j <= amount; ++j) {
                if (coins[i-1] <= j) {
                    // Min of excluding the coin vs. including it (1 + same row)
...
int main() {
    ...
    int amount = 4681;
    cout << "Total no of ways = " << coinChange(amount, coins) << endl;
    return 0;
}